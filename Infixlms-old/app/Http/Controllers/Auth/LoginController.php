...
//        return $request->only($this->username(), 'password');
    ...
    public function loginHistory($request)
    {
        try {
            ...
            $login = UserLogin::create([
                'user_id' => Auth::id(),
                'ip' => $ip,
                'browser' => !empty(Browser::browserName()) ? Browser::browserName() : ($request->browser ?? 'Unknown'),
                'os' => !empty(Browser::platformName()) ? Browser::platformName() : ($request->os ?? 'Unknown'),
                'token' => \session()->getId(),
            ...
            \Log::warning('loginHistory failed: ' . $e->getMessage());
        }
    }
    ...
    protected function sendLoginResponse(Request $request)
    {
        ...
        try { checkGamification('each_login', 'activity'); } catch (\Throwable $e) {}
        try { checkGamificationReg(); } catch (\Throwable $e) {}
