...
if (!function_exists('permissionCheck')) {
    ...
    {
        static $cache = [];
        if (isset($cache[$route_name])) {
            return $cache[$route_name];
        }
        if (auth()->check()) {
            if (auth()->user()->role_id == 1) {
                return $cache[$route_name] = TRUE;
            } else {
                ...
                if (isModuleActive('OrgInstructorPolicy')) {
                    ...
                    return $cache[$route_name] = ($role != null && $role->permissions->contains('route', $route_name));
                } else {
                    $roles = app('permission_list');
                    $role = $roles->where('id', auth()->user()->role_id)->first();
                    return $cache[$route_name] = ($role != null && $role->permissions->contains('route', $route_name));
