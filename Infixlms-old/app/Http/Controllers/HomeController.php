...
class HomeController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        try {
            ...
            // Use DB-level aggregates instead of loading all courses/enrollments into memory.
            $enable_courses = (clone $query4)->where('status', 1)->count();
            $disable_courses = (clone $query4)->where('status', 0)->count();
            $courses = (clone $query4)->where('type', 1)->count();
            $quizzes = (clone $query4)->where('type', 2)->count();
            $classes = (clone $query4)->where('type', 3)->count();

            // Keep the dashboard table lightweight.
            $allCourses = (clone $query4)
                ->where('type', 1)
                ->with(['user:id,name'])
                ->withCount('enrolls')
                ->latest('id')
                ->take(50)
                ->get(['id', 'slug', 'title', 'user_id', 'type']);
